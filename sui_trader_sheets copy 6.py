...
class CryptoExchangeAPI:
    ...
    def send_request(self, method, params=None):
        ...
        # JSON serialize öncesi son kontrol - quantity sonunda ".0" varsa 
        # tam sayı gibi görünmesin diye API'ye gönderilmeden önce doğrudan string olarak tut
        if "quantity" in params and isinstance(params["quantity"], str) and params["quantity"].endswith(".0"):
            logger.info(f"CRITICAL: Preventing conversion issues for quantity ending with .0: {params['quantity']}")
            # Bu sayıyı requests.post'ta JSON dönüşümü sırasında değişmemesi için koruyoruz
            # Mevcut değeri değiştirme, parametre olarak gönder
            
            # Custom JSON serialize etme - quantity değerini doğrudan kullanmak için
            json_data = json.dumps(request_body, cls=NoFloatRoundingEncoder)
            response = requests.post(
                endpoint,
                headers=headers,
                data=json_data,  # json_data kullanıyoruz json=request_body yerine
                timeout=30
            )
        else:
            # Normal durum
    ...
    def sell_coin(self, instrument_name, quantity):
        ...
        logger.info(f"Sending SELL order with params: {json.dumps(params, cls=NoFloatRoundingEncoder)}")
        ...
        logger.info(f"✦ REQUEST ID: {request_id}")
        logger.info(f"✦ API METHOD: {method}")
        logger.info(f"✦ PARAMS: {json.dumps(params, indent=2, cls=NoFloatRoundingEncoder)}")
        logger.info(f"✦ PARAM STRING FOR SIGNATURE: {param_str}")
        logger.info(f"✦ SIGNATURE PAYLOAD: {sig_payload}")
        logger.info(f"✦ SIGNATURE: {signature}")
        logger.info(f"✦ FULL REQUEST: {json.dumps(request_body, indent=2, cls=NoFloatRoundingEncoder)}")
        logger.info("=" * 80)
        
        # Send request
        headers = {'Content-Type': 'application/json'}
        
        # KRITIK: requests.post'un json parametresi yerine data parametresini kullanarak
        # manuel olarak serialize ediyoruz, böylece JSON dönüşümü bizim kontrolümüzde oluyor
        # ve Python'un yerleşik JSON serializerı hiçbir şekilde devreye girmiyor
        json_data = json.dumps(request_body, cls=NoFloatRoundingEncoder)
    ...
    def _sell_in_parts(self, instrument_name, total_quantity, part_size):
        ...
        for i in range(num_parts):
            ...
            # YENİ: Hiçbir şekilde yuvarlama yapmadan, doğrudan ham değeri string'e çevir
            formatted_quantity = str(part_quantity)
            
            # Formatlanmış değeri logla 
            logger.info(f"Selling part {i+1}/{num_parts}: {formatted_quantity} {base_currency} without rounding")
            ...
            # JSON dönüşümünde float sayıları yuvarlamayı engellemek için custom encoder kullanabiliriz
            class NoRoundingEncoder(json.JSONEncoder):
                def encode(self, obj):
                    if isinstance(obj, float):
                        return str(obj)
                    return super(NoRoundingEncoder, self).encode(obj)
                
                def iterencode(self, obj, _one_shot=False):
                    if isinstance(obj, float):
                        return str(obj)
                    return super(NoRoundingEncoder, self).iterencode(obj, _one_shot)
            
            logger.info(f"Sending partial SELL order with params: {json.dumps(params, cls=NoRoundingEncoder)}")
            
            # API isteği için endpoint ve request ID hazırla
            endpoint = "https://api.crypto.com/exchange/v1/private/create-order"
            request_id = int(time.time() * 1000)
            nonce = request_id
            ...
            # Send request - .0 ile biten değerler için özel işlem yapalım
            headers = {'Content-Type': 'application/json'}
            
            if formatted_quantity.endswith('.0'):
                logger.info(f"CRITICAL: Preventing JSON rounding for quantity: {formatted_quantity}")
                # Manuel serialize ederek JSON dönüşümündeki yuvarlamayı engelle
                json_data = json.dumps(request_body, cls=NoRoundingEncoder)
                response = requests.post(
                    endpoint,
                    headers=headers,
                    data=json_data,
            ...
            if response_data.get("code") == 0:
                ...
                if error_code == 213:  # INVALID_ORDERQTY
                    ...
                    if base_currency.upper() == 'BTC':
                        # BTC için alternatif miktarlar - yuvarlama yapmadan ham değer
                        alternative_quantities = [
                            "0.001", "0.01", str(part_quantity * 0.5)
                        ]
                    elif base_currency.upper() in ['BONK', 'SHIB', 'PEPE', 'FLOKI']:
                        ...
                        # Diğer coinler için alternatif miktarlar - yuvarlama yapmadan ham değer
                        alternative_quantities = [
                            "1.0", "5.0", str(part_quantity * 0.5)
                        ]
                    ...
                    for j, alt_qty in enumerate(alternative_quantities):
                        ...
                        # Yeni istek gönder
                        alt_request_id = int(time.time() * 1000)
                        alt_nonce = alt_request_id
                        
                        # Convert params to string 
                        alt_param_str = self.params_to_str(alt_params)
                        
                        # Build signature payload
                        alt_sig_payload = method + str(alt_request_id) + self.api_key + alt_param_str + str(alt_nonce)
                        
                        # Generate signature
                        alt_signature = hmac.new(
                            bytes(self.api_secret, 'utf-8'),
                            msg=bytes(alt_sig_payload, 'utf-8'),
                            digestmod=hashlib.sha256
                        ).hexdigest()
                        ...
                        # API isteği gönder - yuvarlama sorununu engellemek için özel işlem
                        if alt_qty.endswith('.0'):
                            logger.info(f"CRITICAL: Preventing JSON rounding for alternative quantity: {alt_qty}")
                            alt_json_data = json.dumps(alt_request_body, cls=NoRoundingEncoder)
                        ...
                        try:
                            ...
                            logger.error(f"Failed to parse alternative response as JSON: {alt_response.text}")
                            continue
                        
                        if alt_response_data.get("code") == 0:
                            alt_order_id = alt_response_data.get("result", {}).get("order_id")
