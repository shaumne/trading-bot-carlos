    ...
    def sell_coin(self, instrument_name, quantity):
        ...
        try:
            ...
            if quantity_float < 0.1:
                logger.warning(f"Adjusted quantity {quantity_float} too small, using minimum 0.1")
                quantity_float = min(0.1, current_balance_float * 0.95)  # Use minimum or 95% of balance
                
            # Format exactly as working example: X.0
            formatted_quantity = f"{quantity_float:.1f}"
            logger.info(f"Final formatted quantity for sell: {formatted_quantity}")
            
        except (ValueError, TypeError) as e:
            logger.error(f"Error processing balance or quantity: {str(e)}")
            # Fallback to original formatting if conversion fails
            formatted_quantity = f"{float(quantity):.1f}"
        ...
        if response.get("code") == 0:
            ...
            # If still failing, try smaller quantity as last resort
            if error_code == 306 or (isinstance(error_msg, str) and "INSUFFICIENT" in error_msg.upper()):
                logger.warning("Insufficient balance error. Trying with 50% of reported balance...")
                try:
                    # Try with 50% of reported available balance
                    safe_quantity = float(current_balance) * 0.5
                    if safe_quantity < 0.1:
                        logger.error(f"Even 50% of balance ({safe_quantity}) too small, aborting")
                        return False
                        
                    fallback_quantity = f"{safe_quantity:.1f}"
                    params["quantity"] = fallback_quantity
                    logger.info(f"Last attempt with 50% of available balance: {fallback_quantity}")
                    
                    # Retry request
                    retry_response = self.send_request(method, params)
                    
                    if retry_response.get("code") == 0:
                        order_id = retry_response.get("result", {}).get("order_id")
                        if order_id:
                            logger.info(f"Fallback successful! Sold {fallback_quantity} {base_currency}")
                            return order_id
                        return True
                    else:
                        retry_error = retry_response.get("message", retry_response.get("msg", "Unknown error"))
                        logger.error(f"Fallback also failed. Error: {retry_error}")
                except Exception as e:
                    logger.error(f"Error in fallback sell attempt: {str(e)}")
        ...
        try:
            ...
            if status == "ORDER_PLACED":
                ...
                if purchase_price:
                    ...
                    logger.info(f"Set Purchase Price to {purchase_price}")
            ...
            # When position is sold
                ...
                if sell_price:
                    ...
                    logger.info(f"Set Sell Price to {sell_price}")
                
                if quantity:
                    # Update Sell Quantity (column O)
                    self.worksheet.update_cell(row_index, 15, str(quantity))
                    logger.info(f"Set Sell Quantity to {quantity}")
                ...
                # Set Buy Signal to empty (column E) to allow new signals
                self.worksheet.update_cell(row_index, 5, "")
                logger.info(f"Cleared Buy Signal")
        ...
        # SELL signal processing
            ...
            try:
                ...
                if not position_found or not quantity or float(quantity) <= 0:
                    ...
                    self.update_trade_status(row_index, "SELL_FAILED_NO_POSITION")
                    return False
                ...
                if quantity < 0.0001:
                    ...
                    self.update_trade_status(row_index, "SELL_FAILED_QTY_TOO_SMALL")
                    return False
                ...
                if order_filled:
                    ...
                    self.update_trade_status(row_index, "SELL_ORDER_NOT_FILLED")
                    return False
                    
            except Exception as e:
                logger.error(f"Error executing sell for {symbol}: {str(e)}")
                self.update_trade_status(row_index, "SELL_ERROR")
                return False
